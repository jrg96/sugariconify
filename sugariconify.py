...
class SugarIconify:
	...
	def iconify(self, file_path, s=None, f=None, g=False, c=False, o=False, d=None, e=False, v=False, p=None, h=False, m=False, x=False, i=False):
		...
		self.svgdirpath, self.sep, self.svgfilename = self.svgfilepath.rpartition('/')
		...
		if self.use_entities:
			...
			self.stroke_match = re.search(r'stroke_color\s*\"([^"]*)\"', self.svgtext)
			self.fill_match = re.search(r'fill_color\s*\"([^"]*)\"', self.svgtext)
			...
			if self.fill_match and self.stroke_match:
				self.entities  = '\t<!ENTITY ' + self.stroke_entity + ' "' + self.stroke_color + '">\n'
				self.entities += '\t<!ENTITY ' + self.fill_entity   + ' "' + self.fill_color   + '">\n'
				if self.use_iso_strokes:
					self.entities += '\t<!ENTITY ' + self.iso_stroke_entity   + ' "' + self.stroke_color   + '">\n'
			else:
				self.entities  = '\t<!ENTITY ' + self.stroke_entity + ' "' + self.default_stroke_color + '">\n'
				self.entities += '\t<!ENTITY ' + self.fill_entity   + ' "' + self.default_fill_color   + '">\n'
				if self.use_iso_strokes:
					self.entities += '\t<!ENTITY ' + self.iso_stroke_entity   + ' "' + self.default_stroke_color   + '">\n'

			#for simplicity, we simply replace the entire entity declaration block; this obviously would remove
			#any other custom self.entities declared within the SVG, but we assume that's an extreme edge case
			
			self.svgtext, self.n = re.subn(r'(<!DOCTYPE[^>\[]*)(\[[^\]]*\])*\>', r'\1 \n[\n' + self.entities + ']>\n', self.svgtext)

			#add a doctype if none already exists, adding the appropriate self.entities as well
			if self.n == 0:
				self.svgtext, self.n = re.subn("<svg", "<!DOCTYPE svg  PUBLIC '-//W3C//DTD SVG 1.1//EN'  'http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd' [\n" + self.entities + "]>\n<svg", self.svgtext)
				if self.n == 0:
		...
		if self.w != '55px' or self.h != '55px':
			print "Warning: invalid canvas size (%s, %s); Should be (55px, 55px)" % (self.w, self.h)
			
		#guess the entity values, if they aren't passed in

		if self.use_entities:
			if self.entities_passed < 2:
				self.stroke_color, self.fill_color = self.guessEntities(self.svg)
		...
		if self.multiple:
			...
			for icon in icons:
				...
				try:
					...
					if icon.localName == 'g' and icon.attributes:
						...
						icon_name = ''
						try:
							if self.creator == 'inkscape' and icon.attributes.getNamedItem('inkscape:label'):
						...
						if not icon_name.startswith('_'):
							...
							#skip non-matches
							if self.pattern == '' or re.search(self.pattern, icon_name):

								if self.verbose:
									print '\nExporting ' + icon_name + '.self.svg...'
								...
								if self.use_entities:
									strokes_replaced, fills_replaced = self.replaceEntities(icon_xml.childNodes[1])
								...
								#write the file
								try:
									f = open(self.output_path + icon_name + '.self.svg', 'w')
								except:
					...
					if(icon_name):
						print 'Error: Could not export' + icon_name + '.self.svg'
			...
			if not self.overwrite_input:
				outfilename = re.sub(r'(.*\.)([^.]+)', r'\1sugar.\2', self.svgfilename)
				if self.verbose:
			...
			for node in self.svg.childNodes:
				...
				if node.localName == 'g' and node.attributes:        
					try:
						if self.creator == 'inkscape' and node.attributes.getNamedItem('inkscape:label'):
			...
			if self.use_entities:
				strokes_replaced, fills_replaced = self.replaceEntities(self.svgxml)
				if not strokes_replaced and not fills_replaced:
			...
			if self.output_examples:
				...
				example_path = self.output_path + re.sub(r'(.*\.)([^.]+)', r'\1preview', self.svgfilename) + '/'
				try:
				...
				example_colors = [(self.default_stroke_color, "#FFFFFF", self.default_stroke_color), 
						  ("#FFFFFF", self.default_stroke_color, self.default_stroke_color), 
										  ("#0000AA", "#00DD00", "#0000AA")]
				example_filenames = [re.sub(r'(.*\.)([^.]+)', r'\1stroke.\2', self.svgfilename),
							 re.sub(r'(.*\.)([^.]+)', r'\1fill.\2', self.svgfilename),
							 re.sub(r'(.*\.)([^.]+)', r'\1both.\2', self.svgfilename) ]
				...
				for i in range(0, len(example_filenames)):
					...
					try:
						...
						sys.exit('Error: Could not write file ' + self.output_path + example_filenames[i])

			try:
				f = open(self.output_path + outfilename, 'w')
			except:
				sys.exit('Error: Could not save to ' + self.output_path + outfilename)
			...
			try:
				...
				sys.exit('Error: Could not write file ' + self.output_path + outfilename)
	...
	def getFill(self, node):
		...
		if f:
			...
			if re.search(r'fill:', node.getAttribute('style')):
				...
				f2 = re.sub(r'.*fill:\s*rgb\(([^)]*).*', r'\1', node.getAttribute('style'))
				if 'rgb' not in f:
					return f.lower()
				else:
	...
	def replaceEntities(self, node, indent=''):
		...
		if node.nodeType == 1: #only element nodes have attrs
			...
			str = str + " (" + self.getStroke(node) + ", " + self.getFill(node) + ")"
			if self.verbose:
				print str
	...
	#these functions attempt to guess the hex values for the stroke and fill self.entities
